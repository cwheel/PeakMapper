...
var populateDatabase = (state, from, to) => {
	getReports(state, from, to).then((reports) => {
		for (var i = 0; i < reports.length; i++) {
			var id = reports[i].id;

			getReport(state, id).then((report) => {
				if (report.peaks == '') {
